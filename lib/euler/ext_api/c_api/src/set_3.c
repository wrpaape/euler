                ...
                long *perm_count_ptr)
{
  *dig_buff_ptr = digs_head_ptr -> value; // copy next smallest digit to the buffer
  ...
  struct DigitNode *next_head_ptr;
  struct DigitNode *prev_ptr;
  struct DigitNode *curr_ptr;
  struct DigitNode **next_digs_heads;
  int del_i;
  ...
  next_digs_heads = handle_malloc(sizeof(DigitNode *) * num_rem_digs);

  for (del_i = 0, del_i < num_rem_digs, ++del_i) {
    prev_ptr = next_digs_heads[del_i];
