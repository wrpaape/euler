...
void problem_23(char *result_buffer)
{
  ...
  for (base_root_i = 0; base_root_i < 10; ++base_root_i) {
    last_perm_i = base_root_i + 2;

    for (perm_root_i = 9; perm_root_i > last_perm_i; --perm_root_i) {
      /* shift perm root left once */
      perm_digit              = digits[perm_root_i];      /* */
      digits[perm_root_i]     = digits[perm_root_i - 1];
      digits[perm_root_i - 1] = perm_digit;



      
      for (upstream_i = perm_root_i; upstream_i < 9)
    }
 ...
 *                                HELPER FUNCTIONS                                  *
 ************************************************************************************/
void do_permute(char root_i, char **digits, long perm_count)
{
}
