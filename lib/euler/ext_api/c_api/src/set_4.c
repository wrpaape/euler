...
struct IntNode *init_dec_palindromes(void)
{
	...
	for (size_t size_pals = sizeof(struct IntNode) * 5;)
	...
	for (int sig_mag = 10,
		...
		for (sig_base = sig_mag,    last_dig = 1;
		     last_dig < 10;
		     sig_base += sig_delta, last_dig += 2) {



			pal_cap = sig_base + sig_mag;

			pal->nxt = handle_malloc(sizeof(struct IntNode));
			pal = pal->nxt;
			pal->val = sig_base + last_dig;
		}
	}
...
bool is_bin_palindrome(const int n)
{
	...
	for (int lil_shift = 1, big_shift = bit_len - 2;
	     lil_shift < halfway;
	     ++lil_shift,	--big_shift) {

		/*
		 * if small bit is 0 and mirrored big bit is 1 or
		 * if small bit is 1 and mirrored big bit is 0
		 *
		 * n is not palindromic in base 2
		 */
		if (((n >> lil_shift) & 1) ^
		    ((n >> big_shift) & 1))
			return false;
