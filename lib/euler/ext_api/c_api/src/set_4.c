...
void problem_36(char *result_buffer)
{
	...
	for (int pal_i = 0; pal_i < num_odd_dec_pals; ++pal_i, ++pal) {
...
bool is_bin_palindrome(const int n)
{
	...
	const int big_pos = bit_len - 1;

	for (int shift = 1; shift < halfway; ++shift) {
		/*
		 ...
		 */
		if (((n >> shift) & 1) ^ ((n >> (big_pos - shift)) & 1))
			return false;
	}
