...
public class JavaAPI {
	...
	private void solveProblem() {
		try {
			...
			this.solution	 = problemMethod.invoke(null);     // solve problem
			long timeStop	 = System.nanoTime();              // stop clock
			this.timeElapsed = (timeStop - timeStart) / 1_000; // convert diff ns → μs
	...
	private void reportSolution() {
		System.out.format(this.solution.toString() + "\n%d", this.timeElapsed);
	}
