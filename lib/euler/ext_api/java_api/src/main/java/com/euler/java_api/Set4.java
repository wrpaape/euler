...
public abstract class Set4 {
	...
	public static String problem38() {
		...
		PanDigHandler handler = new PanDigHandler();
		...
		while (true) {
			...
			if (n > SEARCH_CEIL) {
				return handler.reportMax();
			}
		}
	}


	private static class PanDigHandler {
		...
		private int nextMagProd;

		private PanDigHandler() {
		...
		private void process(int n, int magN) {
			...
			this.magProd = magN;
			// this.nextProd = 2 * n;
			// this.nextMagProd = magN;

			this.concatDigs[0] = digit;
			this.taken[digit] = true;

			if (digit == maxConcatDigs[0]) {
				concatCautious(n, n % magN, magN / 10);

			} else {
				concatProduct(n, n % magN, magN / 10);
			}
		}

		private void concatCautious(int base, int remProd, int remMag) {
			int digit;
			...
			while (remMag > 0) {
				...
				if (taken[digit] || digit < maxConcatDigs[digI] || digI == 7) {
					reset();
					return;
				}
				...
				if (digit > maxConcatDigs[digI]) {
					concatProduct(remProd % magN, remMag / 10);
					return;
				}
			...
			concatCautious(base, remProd, remMag);
		}


		private void concatProduct(int product, int remMag) {
