  ...
  def self.problem_18(*args)
    ...
    begin
      ...
      1.upto(last_row - 1) do |row|  
        ...
        child_nodes = branches.flat_map do |branch|
          root_sum = branch[:sum]
          root_col = branch[:col]

          forks = [{dir: :left,  step: 0},
                   {dir: :right, step: 1}]

          forks.map! do |fork|
            fork[:col]  = root_col + fork[:step]
            fork[:head] = child_row[fork[:col]]
            fork[:sum]  = fork[:head]
          end

          forks.each do |fork|
            col = fork[:col]
            (row + 1).upto(last_row) do |row|
              col        += fork[:step]
              fork[:sum] += tri[row][col]
            end
          end


          if forks.first[:sum] != forks.last[:sum]
            forks = forks.max_by(1) { |fork| fork[:sum] }
          end

            forks.map do |fork|
              {acc_sum: root_sum, col: root_col + fork[:step]}
            end
        end
      end
