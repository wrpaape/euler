defmodule Euler.Set2 do
  ...
  def diag(max_prod), do: do_diag(max_prod, @grid_list)

  def do_diag(max_prod, []),      do: max_prod
  def do_diag(max_prod, rem_grid) do
    rem_grid
    |> Enum.reduce_while([{0, []}, {@adj_count - 1, []}], fn
      (_, [{@adj_count, diag1}, {-1, diag2}]) ->
        {:halt, [diag1, diag2]}

      (row, diag_tups) ->
        next_diag_tups =
          diag_tups
          |> Enum.map_reduce(1, fn({offset, diag}, inc) ->
            {{offset + inc, [Enum.slice(row, offset, @grid_size - @adj_count) | diag]}, inc * -1}
          end)
          |> elem(0)

        {:cont, next_diag_tups}
    end)
    |> Enum.reduce(max_prod, &do_vert({&1, &2}))
    # |> Enum.reduce(max_prod, fn(diag, max_prod) -> do_vert({diag, max_prod}) end)
    |> do_diag(tl(rem_grid))
  end
