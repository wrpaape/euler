defmodule Euler.Set4 do
  ...
  def problem_40 do
    ...
    |> Stream.unfold(fn
      ...
      ({next_n, n, dpn, num, mag, next_mag}) ->

      dig_step = next_n - n

      num_step  = div(dig_step, dpn)

      next_num = num + num_step

      if next_num < next_mag do

        overshoot = rem(dig_step, dpn)

        dig_mag = Sets.nth_pow(10, overshoot)

      else

        delta_num = next_mag - num

        dig_step = dig_step - (delta_num * dpn)

        num_step = div(dig_step, dpn)

        next_num = next_mag + num_step
      end



      
      delta_num = num_state.next_mag - num_state.num
