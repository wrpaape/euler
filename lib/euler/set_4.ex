defmodule Euler.Set4 do
  ...
  def problem_40 do
    {1, 10, 1, 10, 1, 1}
    |> Stream.unfold(fn
      ({10_000_000, _, _, _, _, _}) -> nil

      (tup = {n, next_n, mag, next_mag, num, dpn}) ->

        IO.inspect(tup)

        next_n_tup = {next_n, next_n * 10}
          
        dig_step = next_n - n

        num + div(dig_step, dpn)
        |> case do
          next_num when next_num < next_mag ->
            ...
            next_state =
              next_n_tup
              |> Tuple.append(mag)
              |> Tuple.append(next_mag)
              |> Tuple.append(next_num)
              |> Tuple.append(dpn)
              ...
              |> case do
                0         ->
                  {div(next_num, mag), next_state}

                overshoot ->

                  mag_digit = Sets.nth_pow(10, overshoot)
          ...
          next_num ->
            ...
            |> case do
              ^next_mag ->
                ...
                dig_step * dpn
                |> case do
                  ^next_n -> {1, Tuple.append(shared_tup, dpn + 1)}
                  _______ -> {9, Tuple.append(shared_tup, dpn + 1)}
                end


              _________ ->

                rem_digs = dig_step - (dpn * (next_mag - num))
                dpn      = dpn + 1
                next_num = next_mag + div(rem_digs, dpn)
                ...
                |> case do
                  0         ->
                    {div(next_num, next_mag), next_state}
