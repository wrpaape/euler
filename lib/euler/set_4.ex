defmodule Euler.Set4 do
  ...
  def problem_40 do
    ...
    |> Stream.transform({1, 10}, fn
      (next_mag, {_mag_num, next_mag}) ->

        {[bubble: next_mag], {next_mag, next_mag * 10}}

        (number, mag_tup = {mag_num, _next_mag}) ->

          {[{number, mag_num}], mag_tup}
    end)
    |> Stream.transform({0, 1, 10}, fn

      ({:bubble, num}, {n, digits_per_num, next_n}) ->

        {[{n, num, num}], {n + digits_per_num,}
