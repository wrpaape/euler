defmodule Euler.Set4 do
  ...
  def problem_40 do
    10
    |> Stream.iterate(&(&1 * 10))
    |> Stream.transform(1, fn(next_n, prev_n) ->
      delta_n = next_n - prev_n

      next_n
      |> case do
        10_000_000 -> {:halt,     delta_n}
        __________ -> {[delta_n], next_n}
      end
    end)
    |> Stream.transform({1, 1, 1, 10}, fn(delta_n, {prev_num, dpn, mag_num, next_mag}) ->
      ...
      ######################################################
        next_num ->
          ...
          rem_delta_n
          |> rem(dpn)
          |> strip_next_digit({next_num, dpn, next_mag, next_mag * 10})
      end
    end)
    |> Enum.reduce(&*/2)
  end
